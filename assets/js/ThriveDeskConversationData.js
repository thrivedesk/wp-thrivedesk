...
export const ThriveDeskConversationData = () => {
    ...
    useEffect(() => {
        if (contactID !== "") {
            apiFetch({path: `thrivedesk/v1/conversations/contact/${contactID}`}).then((data) => {
                        ...
                        <tbody>
                        {
                            conversations.length > 0 ?
                                conversations.map(conversation => {
                                    return (
                                        <tr>
                                            <td className="bwf-table-item">{conversation.id}</td>
                                            <td className="bwf-table-item">{conversation.title}</td>
                                            <td className="bwf-table-item">{conversation.status}</td>
                                            <td className="bwf-table-item">{conversation.submitted_at}</td>
                                            <td className="bwf-table-item"><a className={'bwf-a-no-underline'}
                                                                              href={conversation.action}
                                                                              target='_blank'>View Conversation</a></td>
                                        </tr>
                                    )
                                })
                                :
                                <tr>
                                    <td className="bwf-table-empty-item" colSpan="5">No conversations found</td>
                                </tr>
                        }
                        </tbody>
                    </table>
