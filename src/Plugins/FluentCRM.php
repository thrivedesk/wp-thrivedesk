...
final class FluentCRM extends Plugin {
	...
	public function connect() {
		...
		$thrivedesk_options['fluentcrm']              = $thrivedesk_options['fluentcrm'] ?? array();
	...
	public function disconnect() {
		$thrivedesk_options              = get_option( 'thrivedesk_options', array() );
		$thrivedesk_options['fluentcrm'] = $thrivedesk_options['fluentcrm'] ?? array();
		$thrivedesk_options['fluentcrm'] = array(
			'api_token' => '',
			'connected' => false,
		);
	...
	public function get_plugin_data( string $key = '' ) {
		...
		$options            = $thrivedesk_options['fluentcrm'] ?? array();
	...
	public function get_customer(): array {
		...
		if ( function_exists( 'FluentCrm' ) ) {
			$app       = FluentCrm();
			$countries = $app->applyFilters( 'fluentcrm-countries', array() );
		...
		return array(
			...
			'date_of_birth'  => $this->customer->date_of_birth ? date(
				'd M Y',
				strtotime( $this->customer->date_of_birth )
			) : '',
			'last_activity'  => $this->customer->last_activity ? date(
				'd M Y',
				strtotime( $this->customer->last_activity )
			) : '',
			'updated_at'     => $this->customer->updated_at ? date(
				'd M Y',
	 ...
	 * @since 0.8.4
	 */
	public function sync_conversation_with_fluentcrm( string $syncType, array $extra = array() ): void {
		global $wpdb;
