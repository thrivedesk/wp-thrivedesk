...
class Conversation {
	...
	public function __construct() {
		...
		add_action( 'init', array( $this, 'add_td_conversation_shortcode' ) );

		// ajax call for sending reply
		add_action( 'wp_ajax_td_reply_conversation', array( $this, 'td_send_reply' ) );

		// ajax call for verifying the helpdesk setting
		add_action( 'wp_ajax_thrivedesk_api_key_verify', array( $this, 'td_verify_helpdesk_api_key' ) );

		// ajax call for saving the helpdesk setting
		add_action( 'wp_ajax_thrivedesk_helpdesk_form', array( $this, 'td_save_helpdesk_form' ) );

		add_action( 'wp_ajax_thrivedesk_system_info', array( $this, 'thrivedesk_system_info' ) );
	}
	...
	public function reset_td_settings( $apiKey ): void {
		if ( get_option( 'td_helpdesk_settings' ) ) {
			...
			update_option( 'td_helpdesk_system_info', array() );
		} else {
			add_option(
	...
	public function td_save_helpdesk_form() {
		...
		if ( isset( $data['td_helpdesk_api_key'] ) ) {
			...
			$td_helpdesk_settings = array(
				...
				'td_knowledgebase_slug'    => $data['td_knowledgebase_slug'] ?? array(),
				'td_helpdesk_post_types'   => $data['td_helpdesk_post_types'] ?? array(),
				'td_helpdesk_post_sync'    => $data['td_helpdesk_post_sync'] ?? '',
				'td_user_account_pages'    => $data['td_user_account_pages'] ?? array(),
				'td_assistant_route_list'  => $data['td_assistant_route_list'] ?? array(),
			);
	...
	public function add_td_conversation_shortcode(): void {
		add_shortcode( 'thrivedesk_portal', array( $this, 'conversation_page' ) );
	}
