...
class TDApiService {
	...
	public function getRequest( string $url ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			return array(
				'wp_error' => true,
				'message'  => 'ThriveDesk - API call error:' . $error_message,
			);
		} else {
			...
			if ( 200 === $response_code ) {
				...
				if ( 403 === $response_code || 402 === $response_code ) {
					...
					if ( str_contains( $body, 'Cloudflare' ) ) {
						return array(
							'wp_error' => true,
							'message'  => 'ThriveDesk - API blocked by Cloudflare. ' . $instruction_ip_whitelist,
						);
					}
				}
				...
				return array(
					'wp_error' => true,
					'message'  => 'ThriveDesk - API request failed. Response Code:' . $response_code . '. Message: ' . $body['message'] ?? '',
				);
			}
		}
		...
		return array(
			'wp_error' => true,
			'message'  => 'ThriveDesk - Unknown API request error. Response Code:' . $response_code,
		);
	}
