...
final class Admin {
	...
	public function admin_scripts($hook): void {
		if ('settings_page_thrivedesk-setting' == $hook) {
			wp_enqueue_style('thrivedesk-admin-style', THRIVEDESK_PLUGIN_ASSETS . '/css/admin.css', '',
				THRIVEDESK_VERSION);
		}

		wp_enqueue_script('thrivedesk-admin-script', THRIVEDESK_PLUGIN_ASSETS . '/js/admin.js', ['jquery'],
			THRIVEDESK_VERSION);
		...
		if (class_exists('BWF_Contacts')) {
			...
			wp_enqueue_script('thrivedesk-autonami-script',
				THRIVEDESK_PLUGIN_ASSETS . '/js/wp-scripts/thrivedesk-autonami-tab.js', $asset_file['dependencies'],
				$asset_file['version'] ?? THRIVEDESK_VERSION);
		}
	}
	...
	public function activate(): void {
		...
		if (!$installed) {
			...
			if (false == get_option('thrivedesk_options')) {
				update_option('thrivedesk_options', []);
			}
		}
