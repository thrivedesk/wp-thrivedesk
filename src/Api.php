...
final class Api {
	...
	public function api_listener(): void {
		// Early check: only process if this is actually an API request
		$listener = isset( $_GET['listener'] ) ? sanitize_key( wp_unslash( $_GET['listener'] ) ) : '';
		if ( 'thrivedesk' !== $listener ) {
			return;
		}

		// Early check: only process if nonce is present (indicates API request)
		if ( ! isset( $_GET['_wpnonce'] ) ) {
			return;
		}

		// Verify nonce
		if ( ! wp_verify_nonce( sanitize_key( $_GET['_wpnonce'] ), 'td_api_nonce' ) ) {
	...
	public function register_rest_routes(): void {
		register_rest_route(
			...
			array(
				...
				'permission_callback' => array( $this, 'verify_rest_permissions' ),
				'args'                => array(
					'action' => array(
						'required'          => true,
	...
	public function handle_rest_request( \WP_REST_Request $request ): \WP_REST_Response {
		try {
			...
			$this->item         = sanitize_key( $request->get_param( 'item' ) ?? '' );
			$this->item_id      = sanitize_key( $request->get_param( 'item_id' ) ?? '' );
			$this->coupon       = sanitize_key( $request->get_param( 'coupon' ) ?? '' );
			...
			if ( ! class_exists( $plugin_class_name ) ) {
				return new \WP_REST_Response(
					array( 'error' => "Class not found for the '{$plugin_name}' plugin" ),
					500
				);
			}
			...
			if ( ! method_exists( $this->plugin, 'is_plugin_active' ) ) {
				return new \WP_REST_Response(
					array( 'error' => "Method 'is_plugin_active' not exist in class '{$plugin_class_name}'" ),
					500
				);
			}
			...
			if ( ! $this->plugin->is_plugin_active() ) {
				return new \WP_REST_Response(
					array( 'error' => "The plugin '{$plugin_name}' isn't installed or active." ),
					500
				);
			}
			...
			return new \WP_REST_Response(
				array( 'error' => 'Cannot prepare data: ' . $e->getMessage() ),
				500
			);
		}
	}
	...
	private function execute_action( string $action, \WP_REST_Request $request ): array {
		switch ( $action ) {
			case 'connect':
				$this->plugin->connect();
				return array( 'message' => 'Site connected successfully' );
			...
			case 'get_fluentcrm_data':
				...
				if ( $sync_type ) {
					$extra = $request->get_param( 'extra' ) ?? array();
					$this->plugin->sync_conversation_with_fluentcrm( $sync_type, $extra );
					return array( 'message' => 'Success' );
				} else {
					if ( ! method_exists( $this->plugin, 'prepare_fluentcrm_data' ) ) {
						throw new \Exception( "Method 'prepare_fluentcrm_data' not exist in plugin" );
					}
					if ( ! $this->plugin->is_customer_exist() ) {
						throw new \Exception( 'Customer not found.' );
					}
					return $this->plugin->prepare_fluentcrm_data();
				}
			...
			case 'handle_autonami':
				...
				if ( $sync_type ) {
					...
					if ( ! method_exists( $this->plugin, 'prepare_data' ) ) {
						throw new \Exception( "Method 'prepare_data' not exist in plugin" );
					}
					if ( ! $this->plugin->is_customer_exist() ) {
						throw new \Exception( 'Customer not found.' );
					}
					return $this->plugin->prepare_data();
				}
			...
			case 'get_woocommerce_order_status':
				...
				if ( ! method_exists( $this->plugin, 'order_status' ) ) {
					throw new \Exception( "Method 'order_status' not exist in plugin" );
				}
				...
				return $this->plugin->order_status( $order_id );
			...
			case 'woocommerce_order_apply_coupon':
				$order = wc_get_order( $this->order_id );
				if ( $this->coupon ) {
					$res = $order->apply_coupon( $this->coupon );
					if ( isset( $res->errors ) ) {
			...
			default:
				return $this->get_plugin_data( $request );
		}
	}

	/**
	 * Get plugin data for REST API.
	 *
	 * @param \WP_REST_Request $request The REST request object.
	 * @return array
	 * @since 0.0.1
