...
PHPUnit needs a WordPress test environment: `wp-phpunit` is vendored, but `tests/bootstrap.php` still requires `WP_TESTS_DIR`/`WP_PHPUNIT__DIR` plus a `wp-tests-config.php` pointing at a MySQL database. See `.github/workflows/phpunit.yml` for the exact provisioning steps.
