...
$conversation_data   = isset( $conversations['data'] ) ? Conversation::td_conversation_sort_by_status( $conversations['data'] ) : array();
$links               = $conversations['meta']['links'] ?? array();
	...
	<div class="td-container">
		...
		<?php else : ?>
			...
			<div class="td-portal-footer">
				<a class="flex items-center space-x-2 text-xs cursor-pointer text-slate-600 uppercase opacity-75 hover:opacity-100" href="https://www.thrivedesk.com/wordpress?utm_source=wpportal&utm_medium=<?php echo esc_attr( get_site_url() ); ?>&utm_campaign=powered-by" target="_blank">
					...
					<nav class="ml-auto" aria-label="Page navigation">
						<ul class="td-paginator">
							...
							foreach ( $links as $key => $link ) :
								?>
								<?php
								$params = wp_parse_url( $link['url'] ?? '' )['query'] ?? '';
								if ( $params ) {
