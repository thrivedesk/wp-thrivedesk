	...
	<div class="space-y-4">
		<?php foreach ( $conversation['events'] as $event ) : ?>
			<?php if ( $event['event'] && $event['action'] !== 'note' ) : ?>
				...
				<div class="td-conversation <?php echo esc_attr( $event['actor_type'] == ACTOR_TYPE ? 'actor-contact' : 'actor-agent' ); ?>">
					...
					<div class="td-conversation-body py-4" dir="auto">
						...
						<?php elseif ( $event['event']['text_body'] ) : ?>
							<?php echo esc_html( $event['event']['text_body'] ); ?>
						<?php endif; ?>
					</div>
				</div>
			<?php endif; ?>
		...
		<div>
			<form action="" id="td_conversation_reply" method="POST">
				...
				<?php wp_editor( '', 'td_conversation_editor', array( 'editor_height' => '120' ) ); ?>
