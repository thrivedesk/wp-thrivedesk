...
$td_selected_post_types      = $td_helpdesk_selected_option['td_helpdesk_post_types'] ?? array();
$td_selected_post_sync       = $td_helpdesk_selected_option['td_helpdesk_post_sync'] ?? array();
$td_assistants               = Assistant::assistants();
$td_knowledgebase            = KnowledgeBase::knowledgebase();
$td_api_key                  = isset( $_GET['token'] ) ? sanitize_text_field( wp_unslash( $_GET['token'] ) ) : ( $td_helpdesk_selected_option['td_helpdesk_api_key'] ?? '' );
...
$td_selected_user_account_pages                      = $td_helpdesk_selected_option['td_user_account_pages'] ?? array();
...
$wp_post_sync_types = array_filter(
	get_post_types(
		array(
			'public'       => true,
			'show_in_rest' => true,
...
$knowledge_base_wp_post_types = array_filter(
			...
			<?php if ( ! empty( $td_assistants ) ) : ?>
				...
				<div class="space-y-2">
						...
						<select name="td_excluded_routes[]" id="td-excluded-routes" class="mt-1 bg-gray-50 border border-gray-300 rounded px-2 py-1 w-full max-w-full" multiple>
							<?php
							$selected_routes = $td_helpdesk_selected_option['td_assistant_route_list'] ?? array();
					...
					<small class="text-gray-600 block mt-1">
						<?php echo wp_kses( __( 'Hold down the <strong>Ctrl</strong> (or <strong>Cmd</strong> on Mac) key to select multiple routes.', 'thrivedesk' ), array( 'strong' => array() ) ); ?>
					</small>
